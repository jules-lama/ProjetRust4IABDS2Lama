...
fn redirection(){
    ...
    let _commande2 = Command::new("echo")
        .stdin(commande1_out)
        .spawn()
        .expect("failed to execute process");

    let commande_out = _commande2.wait_with_output().expect("failed to execute process");
    assert_eq!(String::from_utf8_lossy(&commande_out.stdout),"Hello, world!\n");
}
